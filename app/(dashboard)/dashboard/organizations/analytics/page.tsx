  ...
  useEffect(() => {
    async function fetchAnalytics() {
      try {
        ...
        const field = "Research"; // Default field for organizations
        ...
        const data = {
          profile: {
            ...
            field: percentile?.field || "Multi-disciplinary",
            cohortSize: percentile?.total || benchmark?.cohortStats?.fieldScholarsCount || 0,
          },
          mainMetrics: [
            {
              label: "H-Index (Total)",
              value: organizationMetrics?.hIndexTotal?.toString() || pibi?.hIndex?.toString() || "0",
              change: "+3.1%",
              trend: "up",
            },
            {
              label: "H-Index (Last 5Y)",
              value: organizationMetrics?.hIndexLast5?.toString() || cass?.citationsLast5Years?.toString() || "0",
              change: "+2.5%",
              trend: "up",
            },
            {
              label: "World Rank",
              value: organizationMetrics?.worldRank ? `#${organizationMetrics.worldRank}` : percentile?.worldRank ? `#${percentile.worldRank}` : "—",
              change: "+8",
              trend: "up",
            },
            {
              label: "Country Rank",
              value: organizationMetrics?.countryRank ? `#${organizationMetrics.countryRank}` : percentile?.countryRank ? `#${percentile.countryRank}` : "—",
              change: "+3",
              trend: "up",
            },
          ],
          aris: {
            score: aris?.ARIS || 0,
            change: "+4.2%",
            percentile: percentile?.percentile || 80,
            ...
            breakdown: [
              { label: "H-Index Impact", value: aris?.hIndex || organizationMetrics?.hIndexTotal || 0, color: "bg-emerald-500" },
          ...
          percentile: {
            value: percentile?.percentile || 80,
            rank: percentile?.rank || organizationMetrics?.countryRank || 1,
            total: percentile?.total || 1250,
