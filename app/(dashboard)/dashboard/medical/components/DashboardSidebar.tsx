...
interface DashboardSidebarProps {
  activePage?: "overview" | "publications" | "analytics" | "tickets" | "settings";
  isOpen?: boolean;
  onClose?: () => void;
}
    ...
    async function fetchUserData() {
      try {
        ...
        if (json.success && json.data) {
          const { user, scholarProfile } = json.data;
          setUserData({
            name:
              scholarProfile?.data?.name ||
              user?.email?.split("@")[0] ||
              "User",
    ...
    >
      ...
      <nav className="flex-1 px-4 py-6 space-y-1">
        ...
        <SidebarItem
          ...
          href="/dashboard/medical/publications"
        />
        <SidebarItem
        ...
        <SidebarItem
          ...
          href="/dashboard/medical/tickets"
        />
        <SidebarItem
          icon={<Settings size={18} />}
          label="Settings & Privacy"
          active={activePage === "settings"}
          href="/dashboard/medical/settings"
        />
      </nav>
      ...
      <div className="p-5 border-t border-[#E1E4EA] mt-auto">
        <div className="flex items-center gap-3 p-2 rounded-lg hover:bg-[#E1E4EA] cursor-pointer transition-colors">
          ...
          <button
            ...
            className="text-[#525866] hover:text-[#E82323] transition-colors"
          >
            <LogOut size={16} />
          </button>
        </div>
