...
//     }
//   >(null);
//   const [nationciteId, setNationciteId] = useState("NCI-112233");
...
//     // 1. Validate the current step UI
//     const stepErrors = validateStep(activeTab, currentStep);
//     if (Object.keys(stepErrors).length > 0) {
//       setErrors(stepErrors);
//       return;
...
//     // 3. Final submission logic (Step 4)
//     if (!nationciteId) {
//       setErrors({ nationciteId: "NationCite ID is required for admin creation" });
//       return;
//     }
...
//         endpoint = "/api/registration/admin/org-create"; //
//         payload = {
//           nationciteId,
//           name: institutionForm.adminName,
//           domain: institutionForm.domainName,
//           email: institutionForm.email,
//           number: institutionForm.mobile,
//           city: institutionForm.city || "Default City",
//           state: institutionForm.state || "Default State",
//           letterOfAuthorizationUrl: institutionForm.letterOfAuthorizationFile || "https://placeholder.com/doc.pdf",
//           accreditationProofUrl: institutionForm.accreditationProofFile || "https://placeholder.com/doc.pdf",
//         };
//       } else {
//         endpoint = "/api/registration/admin/scholar-create"; //
//         const baseData = {
//           nationciteId,
...
//           payload = { ...baseData, ...medicalForm, mobile: medicalForm.mobile };
//         } else {
//           payload = { 
//             ...baseData, 
//             ...researcherForm, 
//             mobile: "0000000000", // Placeholder if not in UI
...
function AddUserDrawer({ onClose }: { onClose: () => void }) {
  ...
  const handlePrimaryAction = async () => {
    ...
    try {
      ...
      if (activeTab === "institution") {
        // Maps to POST /api/registration/admin/org-create (route.ts)
        endpoint = "/api/registration/admin/org-create";
        payload = {
          nationciteId,
          name: institutionForm.adminName,
          ...
          number: institutionForm.mobile,
          city: institutionForm.city || "Not Specified",
          state: institutionForm.state || "Not Specified",
          // Files are not uploaded here — use placeholder URLs; swap with real upload URLs when file upload API is ready
          letterOfAuthorizationUrl: institutionForm.letterOfAuthorizationFile
            ? `https://placeholder.com/${institutionForm.letterOfAuthorizationFile.name}`
            : "https://placeholder.com/letter-of-authorization.pdf",
          accreditationProofUrl: institutionForm.accreditationProofFile
            ? `https://placeholder.com/${institutionForm.accreditationProofFile.name}`
            : "https://placeholder.com/accreditation-proof.pdf",
        };
      } else if (activeTab === "medical") {
        // Maps to POST /api/registration/admin/scholar-create (route1.ts) with type=MEDICAL
        endpoint = "/api/registration/admin/scholar-create";
        payload = {
          nationciteId,
          type: "MEDICAL",
          name: medicalForm.name,
          email: medicalForm.email.trim(),
          mobile: medicalForm.mobile,
          city: medicalForm.city || "Not Specified",
          state: medicalForm.state || "Not Specified",
          ...
          // Files are not uploaded here — use placeholder URLs; swap with real upload URLs when file upload API is ready
          medicalDegreeUrl: medicalForm.medicalDegreeFile
            ? `https://placeholder.com/${medicalForm.medicalDegreeFile.name}`
            : "https://placeholder.com/medical-degree.pdf",
          regCertificateUrl: medicalForm.regCertificateFile
            ? `https://placeholder.com/${medicalForm.regCertificateFile.name}`
            : "https://placeholder.com/reg-certificate.pdf",
        };
      } else {
        // Maps to POST /api/registration/admin/scholar-create (route1.ts) with type=RESEARCHER
        endpoint = "/api/registration/admin/scholar-create";
        payload = {
          nationciteId,
          type: "RESEARCHER",
          ...
          mobile: "0000000000", // Researcher form has no mobile field — placeholder
          city: researcherForm.city || "Not Specified",
          state: researcherForm.state || "Not Specified",
          institute: researcherForm.institution,
          ...
          // Files are not uploaded here — use placeholder URLs; swap with real upload URLs when file upload API is ready
          profilePhotoUrl: researcherForm.profilePhotoFile
            ? `https://placeholder.com/${researcherForm.profilePhotoFile.name}`
            : "https://placeholder.com/profile-photo.jpg",
          institutionalIdCardUrl: researcherForm.institutionalIdCardFile
            ? `https://placeholder.com/${researcherForm.institutionalIdCardFile.name}`
            : "https://placeholder.com/id-card.pdf",
        };
      }
      ...
      alert(`User created successfully! Credentials sent to ${result.data?.email ?? "their email"}.`);
      onClose();
      window.location.reload(); // Refresh the user list
    } catch (err: any) {
      alert(err.message);
    ...
    <>
      ...
      <div className="drawer-panel">
        ...
        <div className="drawer-footer" style={{ marginBottom: "5px" }}>
          ...
          >
            {Loading && currentStep === 4 ? "Submitting..." : primaryLabel()}
          </button>
        </div>
      </div>
    </>
